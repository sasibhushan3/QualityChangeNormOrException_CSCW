''' This code extracts the entire revision history of the selected list of text pages (main article pages) from wikipedia dumps.
	...
	We have extracted the text page history of 30826 pages. To extract the entire text page history
...
parser = argparse.ArgumentParser(description='Read Arguments for extracting the entire history of text pages')
...
parser.add_argument('--destination_path', type=str, nargs='?', default='./',
                                        help='path of the text pages to be saved')
...
# list of text pages to be extracted
with open('titles.json', 'r') as fp:
	titles = json.load(fp)

titles = ['text:'+i for i in titles]

# Extracting the entire text page history of the given list of pages
for link in links:
	# Download the dump
	os.system('wget '+link)
	dump_name = link[44:]
	...
	# Check only for your list of text pages to be extracted
	dict1 = {}
	names = []
	...
	if(len(ranges) == 0):
		...
		if(extractor != " "):
			...
			with open('pages.xml','rb') as n:
				a = xmltodict.parse(n)
			# If this dump has more than one of the required text pages	
			if(len(names2) > 1):
				for x in a["document"]["page"]:
					...
					page_name = page_name.replace("\'","_")
					if ttl in names2:
						with open(args.destination_path+page_name.replace(" ","_")+'0.pkl','wb') as fi:
							pk.dump(c,fi)
			else:
				# if the dump has only one of the required text page
				x = a["document"]["page"]
				b = dict(x)
				c = json.loads(json.dumps(b))
				ttl = b["title"] # check this if this one from the wikipedia dump and the title list are same or not.
