''' This code runs the Binary Segmentation (BinSeG) CPD algorithm on the time series of features. These features are generated from the given set of wikipedia articles and 
                     their corresponding talk pages. The code computes precision, recall and coverage as the output of BinSeG algorithm.
...
parser = argparse.ArgumentParser(description='Read Arguments for running BinSeG algorithm')
parser.add_argument('--dataset_path', type=str, nargs='?', default='sample_dataset.json',
                                        help='path of the dataset')
parser.add_argument('--n_bkps', type=float, nargs='?', default=3.85,
                                        help='Number of break points for BinSeG algorithm')
...
for page in pages:
	...
	try:
		my_bkps = algo.predict(n_bkps=args.n_bkps)
	except:
		continue
