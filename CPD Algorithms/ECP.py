...
parser = argparse.ArgumentParser(description='Read Arguments for running ECP algorithm')
parser.add_argument('--dataset_path', type=str, nargs='?', default='sample_dataset.json',
                                        help='path of the dataset')
parser.add_argument('--min_size', type=int, nargs='?', default=5,
                                        help='Min Size for ECP algorithm')
...
for page in pages:
	...
	partition = np.array([k for k, l  in enumerate(y) if l == 1])
	for i in np.arange(1, len(partition))[::-1]:
		partition[i] = partition[i] - partition[i-1]

	changes = np.cumsum(partition)
	sol = cd.divisive(data, sig_lvl=0.05, R=100, alpha=1, min_size=args.min_size)

	estimates = []
	for p in sol["estimates"][1:-1]:
		estimates.append(p)
	gts = []
	for p in changes:
		gts.append(p)
